...
module ICU
  describe Locale do
    describe 'the available locales' do
      ...
      it { expect(subject.first).to be_a Locale }
    end
    ...
    describe 'the default' do
      ...
      it { expect(subject).to be_a Locale }
    ...
    describe 'display' do
      context 'in a specific locale' do
        ...
        it 'returns the script' do
          expect(Locale.new('ja_Hira_JP').display_script('en').downcase).to eq 'hiragana'
          expect(Locale.new('ja_Hira_JP').display_script('ru').downcase).to eq 'хирагана'
        end
